...
def test_default_rule_returns_default_modifier(make_context):
    ctx = make_context(["hello"], cursor=Cursor(family="Arial", size=12, style="", colour=[0, 0, 0], fill=[255, 255, 255]))
...
def test_char_rule_returns_current_char(make_context):
    ctx = make_context(["hello"], i=0, j=1, cursor=Cursor(family="Arial", size=12, style="", colour=[0, 0, 0], fill=[255, 255, 255]))
